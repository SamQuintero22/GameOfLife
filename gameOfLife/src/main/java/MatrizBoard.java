...
public class MatrizBoard implements BoardStrategy {
    Cell [][] matriz;
    int totalRows;
    int totalCols;


    public MatrizBoard(int totalRows, int totalCols){
    ...
    public void initializeGridWithDeadCells() {
        for (int i = 0; i < totalRows; i++) {
            ...
            matriz[i][j] = new Cell(i, j,false,Colors.NEUTRO);
            }
        }
    }
    ...
    BoardStrategy newBoard = new MatrizBoard(rows, cols);
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < cols; col++) {
        Cell cell = getCell(row, col);
        Cell newCell = (cell.isAlive()) ? new Cell(row, col, true,Colors.NEUTRO)
            : new Cell(row, col, false,Colors.NEUTRO);
